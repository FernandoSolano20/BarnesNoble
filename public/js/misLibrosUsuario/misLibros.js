...
let mostarLibros = async (event) => {
    if(!event){
        listaLibroUsuario = await verLibrosCompradosLector(sessionStorage.id);
    }
...
let agregarCardLibro = function (libro, ejemplar, infoEjemplar) {
    ...
    let btnEstado = document.createElement('a');
    btnEstado.setAttribute('class', 'material btnLibreria downButton');
    btnEstado.href = "#";
    btnEstado.innerText = 'Desactivar';
    divContainerButtons.appendChild(btnEstado);
